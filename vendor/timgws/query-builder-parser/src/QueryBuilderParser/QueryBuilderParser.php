...
class QueryBuilderParser
{
    ...
    public function parse_subquery($obj){
	 
	$rules=$obj->rules;
	
	foreach ($rules as $r)
	{
		if (isset($r->type)&&$r->type=='subquery')
		{	
			$this->parse_subquery($r->value);
					
			$mod='App\\'.$r->table;
			if (method_exists ($mod,'getJoints'))
			{
				$query = new JoinSupportingQueryBuilderParser(null, $mod::getJoints());
			...
			$q= $query->parse(json_encode($r->value), $mod::query());
			$items=$q->pluck('id');
		...
		else if (isset($r->rules)) {
			$this->parse_subquery($r);
		} 
	}
    ...
    protected function createNestedQuery(Builder $querybuilder, stdClass $rule, $condition = null)
    {
        ...
        return $querybuilder->where(function($query) use (&$rule, &$querybuilder, &$condition) {
