...
class JoinSupportingQueryBuilderParser extends QueryBuilderParser
{
    ...
    private function buildSubclauseQuery($query, $rule, $value, $condition)
    {
		...
		if ($condition=='and') {
			$conditioned='whereHas';
		} else {
			$conditioned='orWhereHas';
		}
        ...
        $query = $query->{$conditioned}($subclause['to_table'],
            /**
             * @param Builder $query
             */
            function(Builder $query) use ($subclause) {

                //$q = $query->where($subclause['to_table'].'.'.$subclause['to_col'], $subclause['from_table'].'.'.$subclause['from_col']);
                            /*     $q = $query->selectRaw(1)
                 ...
                 $q = $query;//->selectRaw(1)
                    //->from($subclause['to_table']);
                    //->join($subclause['from_table'],$subclause['to_col'],$subclause['from_col']."v");            
