...
QueryBuilder.define('subquery', function(options) {
	...
	this.status.load_modal=options.load_modal;					//new
	this.status.load_func=options.load_func;					//new
...
QueryBuilder.extend({
    ...
    createSubquery: function(rule, name) {
        ...
        rule.load_modal=this.status.load_modal;
        rule.load_func=this.status.load_func;
    ...
    initSubquery: function(rule) {
        ...
        var opts=$.extend({},this.settings) ; //new
       
        opts= $.extendext(true, 'replace', opts, rule.filter.subquery); // new: opts, old: {}
               
        var $b = $('#' + rule.subquery_id);
        $b.attr('data-subquery', rule.subquery_id);
                
        //new
        $c = $('<button type="button" class="btn btn-xs btn-info" data-visible="true"><i class="glyphicon glyphicon-eye-open"></i></button>');	//new     
        $c.prependTo(rule.$el.find(Selectors.rule_actions));
        
        											
        $c.on("click", function() {
			$(this).find('i').toggleClass('glyphicon-eye-open').toggleClass('glyphicon-eye-close');
			$(this).parent().parent().siblings(".rule-value-container").toggle();
		});
		
		if (rule.filter.tab!=null) {
			$c = $('<button type="button" class="btn btn-xs btn-warning" data-toggle="modal" data-target="#'+rule.load_modal+'" data-dest="#'+rule.subquery_id+'"><i class="glyphicon glyphicon-floppy-disk"></i></button>');	//new     
			$c.prependTo(rule.$el.find(Selectors.rule_actions));
               											
			$c.on("click", function() {
				rule.load_func(rule.filter.tab,$(this).data('dest'),rule.filter.label);
			});
		}
		//end new        
		...
		$b.on('afterUpdateGroupCondition.queryBuilder', function(g,c) {		//new
			c.$el.trigger('change')
			});	
...
QueryBuilder.regional['en'] = {
  ...
  "errors": {
    ...
    "subquery_invalid": "Error in subquery"
  },
  "invert": "Invert",
  "NOT": "NOT"
};
QueryBuilder.regional['it'] = {
  ...
  "operators": {
    ...
    "not_between": "non compreso fra"
  },
  "errors": {
    "no_filter": "Occorre scegliere un filtro",
    "empty_group": "Il gruppo è vuoto",
    "radio_empty": "Occorre scegliere un'opzione",
    "checkbox_empty": "Occorre scegliere un'opzione",
    "select_empty": "Occorre scegliere un'opzione",
    "string_empty": "Occorre inserire un contenuto",
    ...
    "number_wrong_step": "Deve essere un multiplo di {0}",
    "datetime_empty": "Occorre inserire un valore",
    "datetime_invalid": "Formato della data non valido({0})",
    ...
    "subquery_invalid": "Errore nella sotto-query"
  },
  "invert": "Inverti",
  "NOT": "NON"
};
