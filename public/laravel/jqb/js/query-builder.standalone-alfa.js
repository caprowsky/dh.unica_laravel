...
QueryBuilder.extend({
    ...
    initSubquery: function(rule) {
		...
		$c = $('<button type="button" class="btn btn-xs btn-warning" data-dest="#'+rule.subquery_id+'"><i class="glyphicon glyphicon-floppy-disk"></i></button>');	//new     
        $c.prependTo(rule.$el.find(Selectors.rule_actions));
               											
        $c.on("click", function() {
			rule.filter.load($(this).attr('dest'));
		});
